"""
U-Net with Attention Gate - Complete Implementation
...
class AttentionGate(nn.Module):
    ...
    def forward(self, g, x):
        """
        g: gating signal dari decoder (lower resolution)
        x: skip connection dari encoder (higher resolution)
        """
        g1 = self.W_g(g)
        x1 = self.W_x(x)
        psi = self.relu(g1 + x1)
...
class UNetAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx in range(0, len(self.decoder), 2):
            ...
            skip = self.attention_gates[idx//2](g=x, x=skip)
...
def train_attention(args):
    ...
    print(f"\n{'='*80}")
    print("TRAINING U-NET ATTENTION GATE")
    print(f"{'='*80}")
    print(f"Device: {device}")
    
    # Model
    model = UNetAttention(in_channels=3, out_channels=1).to(device)
...
def evaluate_attention(args):
    """Evaluasi U-Net Attention Gate dengan visualisasi"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n{'='*80}")
    print("EVALUASI U-NET ATTENTION GATE")
    print(f"{'='*80}")
    ...
    # Print results
    metrics.print_metrics("U-Net Attention Gate")
    ...
    if args.visualize:
        ...
        print(f"\n✓ Visualisasi selesai!")
        print(f"✓ Hasil disimpan di: {args.vis_dir}/UNet_Attention/")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='U-Net Attention Gate - Training & Evaluation')
    ...
    # Checkpoint
    parser.add_argument('--checkpoint_path', type=str, default='checkpoints/unet_attention.pth.tar')
    
    # Evaluation params
    parser.add_argument('--threshold', type=float, default=0.5)
    parser.add_argument('--output_csv', type=str, default='results/attention_metrics.csv')
